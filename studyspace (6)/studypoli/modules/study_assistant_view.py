...
def render_study_assistant_view(dm):
        ...
        if not tasks:
            ...
            with col_todo:
                ...
                for t in pending_tasks:
                    ...
                    with col_chk:
                        if st.button("✅ Marcar", key=f"toggle_t_{t['id']}", help="Marcar como concluída"):
                            dm.toggle_task(t["id"])
                            st.rerun()
                    with col_txt:
                        st.markdown(f"""
                        <div style="background: rgba(27,42,74,0.045); padding: 8px 12px; border-radius: 8px; border-left: 4px solid {s.get('color', '#3B82F6')}; margin-bottom: 6px;">
                            <span style="font-weight: 600; color: #0F1B33;">{t['text']}</span>
                            <div style="font-size: 0.75rem; color: #253A5C; margin-top: 2px;">
                                {s.get('name')} · <span style="color: {p_col}; font-weight: 700;">{t.get('priority')}</span>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                    with col_del:
                        if st.button("🗑️ Excluir", key=f"del_t_{t['id']}", help="Excluir"):
                            dm.delete_task(t["id"])
                            st.rerun()
            ...
            with col_done:
                ...
                for t in done_tasks:
                    ...
                    col_chk, col_txt, col_del = st.columns([1, 8, 1])
                    with col_chk:
                        if st.button("↩️ Reabrir", key=f"untoggle_t_{t['id']}", help="Reabrir tarefa"):
                            dm.toggle_task(t["id"])
                            st.rerun()
                    with col_txt:
                        st.markdown(f"""
                        <div style="background: rgba(27,42,74,0.04); padding: 8px 12px; border-radius: 8px; margin-bottom: 6px; text-decoration: line-through; color: #1F2E4D;">
                            {t['text']} <span style="font-size: 0.75rem;">({s.get('name')})</span>
                        </div>
                        """, unsafe_allow_html=True)
                    with col_del:
                        if st.button("🗑️ Excluir", key=f"del_done_t_{t['id']}", help="Excluir"):
                            dm.delete_task(t["id"])
                            st.rerun()

    # --- ABA 2: POMODORO & FOCO ANTI-DISTRAÇÃO ---
    with tab_pomodoro:
        st.markdown("#### Cronômetro Pomodoro & Modo Anti-Celular")
        ...
        with cp2:
            st.markdown(f"""
            <div style="background: rgba(27,42,74,0.07); border: 2px solid #2563EB; border-radius: 20px; padding: 30px; text-align: center; box-shadow: 0 0 25px rgba(96, 165, 250, 0.2);">
                <div style="font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.1em; color: #253A5C;">Ciclo Atual</div>
                <div style="font-size: 3.5rem; font-weight: 800; font-family: 'JetBrains Mono', monospace; color: #0F1B33; margin: 10px 0;">
            ...
            st.write("")
            st.caption("Depois de iniciar, mantenha esta aba aberta até o fim — o cronômetro roda em tempo real.")
            ...
            if st.button("▶️ Iniciar Timer", use_container_width=True):
                ...
                for remaining in range(target_secs, -1, -1):
                    ...
                    <div style="text-align: center; font-size: 3rem; font-weight: 800; font-family: 'JetBrains Mono', monospace; color: #0F1B33;">
                        {mins:02d}:{secs:02d}
                    </div>
                    """, unsafe_allow_html=True)
    ...
    with tab_ai_advisor:
        ...
        with col_diag2:
            ...
            st.markdown("""
            <div style="background: rgba(59, 130, 246, 0.1); border: 1px dashed #3B82F6; border-radius: 10px; padding: 12px; margin-top: 14px; font-size: 0.85rem; color: #1D4ED8;">
