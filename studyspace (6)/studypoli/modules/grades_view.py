...
def render_grades_view(dm):
    ...
    if not subjects:
        st.warning("Nenhuma matéria cadastrada. Adicione disciplinas na aba 'Faltas e Disciplinas'.")
        return
        ...
        with st.container():
            ...
            <div style="background: rgba(27,42,74,0.06); border-radius: 14px; padding: 20px; 
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
                    <div>
                        <span style="font-size: 1.2rem; font-weight: 800; color: #0F1B33;">{s['name']}</span>
                        <span style="color: #253A5C; font-size: 0.85rem; margin-left: 8px;">({s.get('code', '---')})</span>
                    </div>
                    <div style="font-size: 0.85rem; color: #1F2E4D;">
            ...
            with col_evals:
                ...
                for idx, ev in enumerate(evals):
                    ...
                    with ce4:
                        done_check = st.checkbox("Feita", value=is_done, key=f"done_{subj_id}_{idx}", label_visibility="collapsed", help="Marcar avaliação como feita")
                ...
                if gstats["is_already_passed"]:
                    ...
                    if gstats["is_impossible"]:
                        ...
                        <div style="background: rgba(239, 68, 68, 0.15); border: 1px solid #EF4444; border-radius: 10px; padding: 14px; color: #B91C1C;">
                             <strong>Atenção Crítica:</strong> Para passar direto com média {gstats['passing_grade']}, seria necessário tirar <strong>{needed}</strong> nas provas restantes (acima de 10).
                            <br><br>
