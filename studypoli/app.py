...
with st.sidebar:
    ...
    st.markdown(f"""
    <div style="background: rgba(30, 41, 59, 0.6); border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; padding: 10px 14px; margin: 10px 0;">
        <div style="font-size: 0.75rem; text-transform: uppercase; color: #94A3B8; font-weight: 700;">Estudante</div>
        <div style="font-size: 1.05rem; font-weight: 800; color: #F8FAFC;">{settings.get('student_name', 'Meu Nome')}</div>
        <div style="font-size: 0.8rem; color: #60A5FA;">🏛️ {settings.get('institution_name', 'Minha Instituição')}</div>
    </div>
    """, unsafe_allow_html=True)

    with st.popover("✏️ Personalizar Meu Perfil"):
    ...
    menu_options = [
        ...
        "⚠️ Matérias e Controle de Faltas",
    ...
    if risky_subjects:
        st.markdown(f"""
        <div style="background: rgba(239, 68, 68, 0.2); border: 1px solid #EF4444; border-radius: 8px; padding: 10px; font-size: 0.8rem; color: #FCA5A5; margin-top: 10px;">
            ⚠️ <strong>Alerta de Frequência!</strong><br>
...
<div class="header-container">
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
        ...
        <div style="text-align: right; margin-top: 8px;">
            ...
            <span class="badge badge-safe">🏛️ {settings.get('institution_name', 'Minha Instituição')}</span>
...
elif selected_page == "⚠️ Matérias e Controle de Faltas":
