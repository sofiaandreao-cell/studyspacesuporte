...
def render_attendance_view(dm):
    st.markdown("## Controle de Faltas por Matéria")
    ...
    with c2:
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-value" style="color: #059669;">{safe_count}</div>
    ...
    with c3:
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-value" style="color: #B45309;">{warning_count}</div>
    ...
    with col_presets:
        with st.expander("Modelos Prontos ou Começar do Zero", expanded=False):
            ...
            with cp_col2:
                if st.button("📋 Modelo Geral", use_container_width=True, help="Matemática, Comunicação e Pesquisa"):
                    ...
                    st.rerun()
            with cp_col3:
                if st.button("📋 Modelo Engenharia", use_container_width=True, help="Cálculo, Física, Álgebra e Computação"):
    ...
    if not subjects:
        ...
        <div style="background: rgba(59, 130, 246, 0.1); border: 2px dashed rgba(96, 165, 250, 0.4); border-radius: 14px; padding: 30px; text-align: center; margin: 20px 0;">
            ...
            <div style="font-size: 1.25rem; font-weight: 700; color: #0F1B33;">Nenhuma matéria cadastrada ainda!</div>
            <div style="color: #253A5C; font-size: 0.9rem; margin-top: 6px;">
        ...
        with st.container():
            ...
            with col_info:
                ...
                if stats["status"] == "failed":
                    alert_box = f"""
                    <div style="background: rgba(220, 38, 38, 0.2); border: 1px solid #DC2626; color: #B91C1C; padding: 6px 12px; border-radius: 8px; font-size: 0.8rem; margin-top: 8px;">
                    ...
                    alert_box = f"""
                    <div style="background: rgba(239, 68, 68, 0.15); border: 1px solid #EF4444; color: #B91C1C; padding: 6px 12px; border-radius: 8px; font-size: 0.8rem; margin-top: 8px;">
                         <strong>Perigo de Reprovação:</strong> Você só pode faltar mais <strong>{stats['remaining_allowed']} aula(s)</strong> no semestre!
                    </div>
                    """
                elif stats["status"] == "warning":
                    alert_box = f"""
                    <div style="background: rgba(245, 158, 11, 0.15); border: 1px solid #F59E0B; color: #B45309; padding: 6px 12px; border-radius: 8px; font-size: 0.8rem; margin-top: 8px;">
                         <strong>Fique Alerta:</strong> Mais da metade das faltas permitidas já foram utilizadas. Restam <strong>{stats['remaining_allowed']} faltas</strong>.
                    </div>
                    """
                else:
                    alert_box = f"""
                    <div style="background: rgba(16, 185, 129, 0.15); border: 1px solid #10B981; color: #047857; padding: 6px 12px; border-radius: 8px; font-size: 0.8rem; margin-top: 8px;">
                         <strong>Frequência Confortável:</strong> Você ainda tem direito a <strong>{stats['remaining_allowed']} falta(s)</strong> restantes.
                    </div>
                    """

                card_html = f"""
                <div style="background: rgba(27,42,74,0.05); border-left: 5px solid {subj.get('color', '#3B82F6')}; 
                            border-radius: 12px; padding: 16px; margin-bottom: 12px; border-top: 1px solid rgba(27,42,74,0.10);">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="font-weight: 700; font-size: 1.15rem; color: #0F1B33;">
                            {subj['name']} <span style="font-size: 0.8rem; color: #253A5C; font-weight: normal;">({subj.get('code', '---')})</span>
                        </span>
                    ...
                    <div style="font-size: 0.85rem; color: #253A5C; margin: 4px 0 10px 0;">
                        Docente: {subj.get('professor', 'Não informado')} · Total de Aulas: {stats['total_classes']} · Média Mínima: {subj.get('passing_grade', 6.0)}
                    </div>
                    <div style="display: flex; justify-content: space-between; font-size: 0.85rem; color: #1F2E4D; font-weight: 600;">
            ...
            with col_controls:
                ...
                with st.popover("Personalizar / Editar Matéria"):
                    st.markdown(f"##### Editar Detalhes de: **{subj['name']}**")
    ...
    if subjects:
        ...
        fig.update_layout(
            ...
            plot_bgcolor="rgba(27,42,74,0.04)",
