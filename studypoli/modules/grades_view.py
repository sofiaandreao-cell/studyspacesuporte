...
def render_grades_view(dm):
    ...
    with c3:
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-value" style="color: #60A5FA;">{pending_count}</div>
    ...
    with c4:
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-value" style="color: #F87171;">{risk_count}</div>
    ...
    for s, gstats in subject_stats_list:
        ...
        with st.container():
            st.markdown(f"""
            <div style="background: rgba(30, 41, 59, 0.7); border-radius: 14px; padding: 20px; 
                        margin-bottom: 20px; border: 1px solid rgba(255,255,255,0.08); border-left: 6px solid {s.get('color', '#3B82F6')};">
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
                    <div>
                        <span style="font-size: 1.2rem; font-weight: 800; color: #F8FAFC;">{s['name']}</span>
                        <span style="color: #94A3B8; font-size: 0.85rem; margin-left: 8px;">({s.get('code', '---')})</span>
                    </div>
                    <div style="font-size: 0.85rem; color: #CBD5E1;">
                        Nota de Corte: <strong>{s.get('passing_grade', 5.0)}</strong> · Média Atual (Feitas): <strong style="color: #60A5FA;">{gstats['current_avg']}</strong>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            with col_sim:
                ...
                if gstats["is_already_passed"]:
                    ...
                    if gstats["is_impossible"]:
                        st.markdown(f"""
                        <div style="background: rgba(239, 68, 68, 0.15); border: 1px solid #EF4444; border-radius: 10px; padding: 14px; color: #FCA5A5;">
                        ...
                        st.markdown(f"""
                        <div style="background: rgba(245, 158, 11, 0.15); border: 1px solid #F59E0B; border-radius: 10px; padding: 14px; color: #FCD34D;">
                        ...
                        st.markdown(f"""
                        <div style="background: rgba(16, 185, 129, 0.15); border: 1px solid #10B981; border-radius: 10px; padding: 14px; color: #6EE7B7;">
                        ...
                        st.markdown(f"""
                        <div style="background: rgba(59, 130, 246, 0.15); border: 1px solid #3B82F6; border-radius: 10px; padding: 14px; color: #93C5FD;">
