...
def render_calendar_view(dm):
    ...
    if not subjects:
        st.warning("Cadastre pelo menos uma matéria na aba 'Matérias e Controle de Faltas' antes de adicionar provas.")
    ...
    st.markdown("#### Próximas Avaliações")
        ...
        with col_info:
            ...
            <div style="background: rgba(27,42,74,0.05); border-left: 5px solid {s.get('color', '#3B82F6')};
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
                    <span style="font-weight: 700; font-size: 1.05rem; color: #0F1B33;">{e['title']}</span>
                ...
                <div style="font-size: 0.85rem; color: #253A5C; margin: 4px 0;">
                ...
                {f'<div style="font-size: 0.8rem; color: #1F2E4D; margin-top: 6px;">{e["topics"]}</div>' if e.get('topics') else ''}
            </div>
            """, unsafe_allow_html=True)
        with col_actions:
            st.markdown("<div style='height: 14px;'></div>", unsafe_allow_html=True)
            if st.button("🗑️ Excluir", key=f"del_exam_{e['id']}", help="Excluir avaliação"):
                dm.delete_exam(e["id"])
                st.rerun()
