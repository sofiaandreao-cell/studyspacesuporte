    ...
    for e in exams:
        ...
        try:
            ...
            if days_left < 0:
                ...
                days_color = "#EF4444" if days_left <= 7 else "#60A5FA"
        except Exception:
        ...
        with col_info:
            st.markdown(f"""
            <div style="background: rgba(30, 41, 59, 0.6); border-left: 5px solid {s.get('color', '#3B82F6')};
                        border-radius: 12px; padding: 14px 18px; margin-bottom: 10px;">
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
                    <span style="font-weight: 700; font-size: 1.05rem; color: #F8FAFC;">{e['title']}</span>
                ...
                {f'<div style="font-size: 0.8rem; color: #CBD5E1; margin-top: 6px;">📖 {e["topics"]}</div>' if e.get('topics') else ''}
            </div>
            """, unsafe_allow_html=True)
        with col_actions:
